...
x=RR(1:length(RR)-1); % intervalo n
y=RR(2:length(RR));   % intervalo n+1
...
% Modificado --
dosRR = mean(RR);
Di_min = sum(x-y)/sqrt(2); % El sumatorio es de N-1, siendo N = length(RR)
Di_maj = sum(x+y-2*dosRR)/sqrt(2);

poincare.SD1 = sqrt((1/length(x))*Di_min^2);  % length(x) = N-1
poincare.SD2 = sqrt((1/length(x))*Di_maj^2);
...
% MODIFICACION ---
xe=xc+sqrt(2)/2*poincare.SD2*cos(t)-sqrt(2)/2*poincare.SD1*sin(t);
ye=yc+sqrt(2)/2*poincare.SD2*cos(t)+sqrt(2)/2*poincare.SD1*sin(t);
% ----
